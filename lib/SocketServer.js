...
function SocketServer(server) {
    ...
    var io = require('socket.io')(server,{perMessageDeflate: false, cors: {
    origin: '*',
  },allowEIO3: true});
    ...
    function init() {
        io.on('connection',(socket) => {
            socket.on('getMapping', (request) =>{
		    logger.info("getMapping",request);
                if (!config.auth.useAuthentication) {
                    if (request.userId || (request.groupId && !request.appId)) {
                        that.emit('getMapping', request, (response) => {
    ...
    function createNameSpace(path) {
        ...
        nsp.on('connection',  (socket) =>{
            ...
            function onGetInitState(data) {
                if (checkIfAllowed()) {
                    that.emit('getState', path, data,  (datagram) => {
            ...
            function onJoin(data) {
                ...
                that.emit('join', path, data,  (allowed, isGroup) => {
